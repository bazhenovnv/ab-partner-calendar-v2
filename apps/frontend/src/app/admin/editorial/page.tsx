...
function ViewsChart({ dashboard, channels }: { dashboard: Dashboard | null; channels: Channel[] }) {
  ...
  const maxValue = Math.max(
    1,
    ...points.flatMap((point) => Object.values(point.channels)),
  );
      ...
      <div className={styles.legend}>
        {channels.map((channel) => (
          <span className={styles.legendItem} key={channel.key}>
            ...
            {channel.name}{!channel.capabilities.nativeViews ? ' — native views недоступны Bot API' : ''}
          </span>
        ))}
      </div>
    </>
...
export default function EditorialPage() {
  ...
  const sanitizedPreview = useMemo(
    () => DOMPurify.sanitize(contentHtml, {
      ALLOWED_TAGS: ['b', 'strong', 'i', 'em', 'u', 's', 'strike', 'del', 'a', 'br', 'p', 'div', 'ul', 'ol', 'li', 'blockquote', 'code', 'pre'],
      ALLOWED_ATTR: ['href', 'target', 'rel'],
    }),
    [contentHtml],
  );

  const selectedPreviewChannels = channels.filter(
    (channel) => channel.platform === previewPlatform && selectedChannels.includes(channel.key),
  );
  ...
  async function saveDraft() {
    ...
    const canUpdate = currentPostId && ['DRAFT', 'FAILED', 'PARTIAL_FAILED'].includes(currentStatus);
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>Редактор публикаций TG / MAX</h1>
          <p className={styles.subtitle}>
            Единый кабинет для новостей и событий: форматированный текст, изображения по шаблонам,
            эмодзи, выбор одного или нескольких каналов, предпросмотр, контроль ошибок и статистика.
          </p>
        </div>
      ...
      <section className={styles.topGrid}>
        <div className={styles.panel}>
          <div className={styles.panelTitleRow}>
            <h2 className={styles.panelTitle}>Просмотры публикаций · 30 дней</h2>
          </div>
      ...
      <section className={styles.editorLayout}>
        <div className={styles.editorPanel}>
          ...
          <div className={styles.formBody}>
            ...
            <div className={styles.field}>
              ...
              <div className={`${styles.charCounter} ${(title.length + contentText.length) > 3700 ? styles.charCounterWarn : ''}`}>
                {title.length + contentText.length} / 3900 символов для совместимой публикации TG + MAX
              </div>
              {emojiOpen && (
            ...
            <div className={styles.field}>
              ...
              <div className={styles.mediaTop}>
                ...
                <label className={styles.fileLabel}>
                  ...
                  <input className={styles.fileInput} type="file" accept="image/*" multiple disabled={uploading} onChange={(event) => uploadImages(event.target.files)} />
                </label>
                <span className={styles.muted}>Сервер сам обрежет копию под выбранный формат; оригинал на компьютере не меняется.</span>
              </div>
              {media.length > 0 && (
                <div className={styles.mediaGrid}>
                  {media.map((item, index) => (
                    <div className={styles.mediaCard} key={`${item.url}-${index}`}>
                      ...
                      <img className={styles.mediaImage} src={item.url} alt={`Изображение ${index + 1}`} />
                      ...
                      <div className={styles.mediaMeta}>{item.templateLabel || item.template}{item.width && item.height ? ` · ${item.width}×${item.height}` : ''}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className={styles.field}>
              ...
              <div className={styles.channelGrid}>
                {channels.map((channel) => {
                  ...
                  return (
                    <label key={channel.key} className={`${styles.channelCard} ${selected ? styles.channelCardSelected : ''} ${!channel.configured ? styles.channelCardDisabled : ''}`}>
                      <input className={styles.channelCheck} type="checkbox" checked={selected} onChange={() => toggleChannel(channel.key)} />
                      <span style={{ minWidth: 0 }}>
                        <span className={styles.channelName}>{channel.name}</span>
                        <span className={styles.channelUrl}>{channel.publicUrl}</span>
                        {!channel.configured && <span className={styles.channelWarning}>{channel.configurationHint || 'Канал пока не настроен. При публикации кабинет покажет точную ошибку конфигурации.'}</span>}
                      </span>
                    </label>
                  );
                })}
          ...
          <div className={styles.formActions}>
            ...
            <button className={styles.buttonPrimary} type="button" onClick={handlePublish} disabled={publishing || saving}>{publishing ? 'Размещение…' : `Проверено — разместить (${selectedChannels.length})`}</button>
          </div>
        </div>

        <aside className={styles.previewPanel}>
          <div className={styles.panelTitleRow}>
            <h2 className={styles.panelTitle}>Предварительный просмотр</h2>
            <span className={styles.muted}>{previewPlatform === 'TELEGRAM' ? 'Telegram' : 'MAX'}</span>
          </div>
          <div className={styles.previewTabs}>
            <button className={`${styles.previewTab} ${previewPlatform === 'TELEGRAM' ? styles.previewTabActive : ''}`} type="button" onClick={() => setPreviewPlatform('TELEGRAM')}>Telegram</button>
            <button className={`${styles.previewTab} ${previewPlatform === 'MAX' ? styles.previewTabActive : ''}`} type="button" onClick={() => setPreviewPlatform('MAX')}>MAX</button>
          </div>
          <div className={styles.phone}>
            <div className={styles.phoneHead}>
              <span className={styles.avatar}>{previewPlatform === 'TELEGRAM' ? 'TG' : 'M'}</span>
              <div>
                <strong>{selectedPreviewChannels.length ? selectedPreviewChannels.map((channel) => channel.name.replace(/^Telegram — |^MAX — /, '')).join(' / ') : `${previewPlatform} — канал не выбран`}</strong>
                <div className={styles.muted}>Предпросмотр публикации</div>
              </div>
            </div>
                  ...
                  {media.length > 0 && (
                    <div className={styles.previewMediaGrid}>
                      {media.slice(0, 4).map((item, index) => (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={item.url} alt="" key={`${item.url}-preview-${index}`} />
                      ))}
                    </div>
                  )}
          ...
          <div className={styles.notice}>
            Предпросмотр максимально приближен к каналам, но Telegram и MAX сами применяют свои шрифты и интервалы. Поддерживаемые стили: жирный, курсив, подчёркивание, зачёркивание, цитата, код и ссылки.
          </div>
            ...
            <div className={publishResult.ok ? styles.successBox : styles.resultBox}>
              ...
              {publishResult.results.map((result) => (
                <div className={styles.resultRow} key={result.channelKey}>
                  <span>{result.channelName}<br />{!result.success && <small>{result.errorCode}: {result.errorMessage}</small>}</span>
        ...
        <div className={styles.panel}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className={styles.table}>
              <thead><tr><th>Тип</th><th>Название</th><th>Статус</th><th>Каналы</th><th>Изменено</th><th /></tr></thead>
              <tbody>
                ...
                {!recentPosts.length && <tr><td colSpan={6} className={styles.muted}>Публикаций пока нет.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
          ...
          <div className={styles.errorList}>
            {(dashboard?.recentErrors ?? []).map((item) => (
              <div className={styles.errorItem} key={item.id}>
                <strong>{item.channelName} · {item.errorCode || 'ERROR'}</strong>
