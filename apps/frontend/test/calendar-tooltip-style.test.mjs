...
describe('Calendar date tooltip styling', () => {
  ...
  test('uses a 20-percent-transparent light surface with slightly larger text', () => {
    const styles = readFileSync(STYLES, 'utf8');

    assert.match(
      styles,
      /\.pub-calendar-tooltip\s*\{[\s\S]*background: rgba\(255, 255, 255, 0\.8\) !important/,
    );
    assert.match(
      styles,
      /\.pub-calendar-tooltip\s*\{[\s\S]*font-size: 13px !important/,
    );
    assert.match(styles, /backdrop-filter: blur\(8px\)/);
    assert.match(
      styles,
      /\.pub-calendar-tooltip::after\s*\{[\s\S]*border-top-color: rgba\(255, 255, 255, 0\.8\) !important/,
    );
  });
});
