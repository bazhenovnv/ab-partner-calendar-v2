...
Content-Security-Policy is added at the nginx level (not in Next.js headers config) because all traffic passes through nginx in production, keeping the policy in one place. The policy uses `unsafe-inline` and `unsafe-eval` in `script-src` because Next.js App Router requires them for hydration inline scripts. `mc.yandex.ru` and `yastatic.net` are whitelisted for Yandex Metrika. `img-src https:` is used broadly to allow event images from external CDNs. The `default.conf` file is dev-only (HTTP, `server_name _`) and is explicitly NOT mounted in production — only `prod.conf` is mounted as `/etc/nginx/conf.d/default.conf` via docker-compose.prod.yml. The `test.ab-event.pro` 443 block requires a separate TLS certificate. To avoid nginx startup failure when that cert does not exist, the 443 block is commented out; test.ab-event.pro is served over HTTP only until the cert is issued. The HTTP redirect for test.ab-event.pro was removed from the port 80 block to avoid redirect loops. A commented-out HTTPS block with a TODO is included for when the cert becomes available.

## ADR-014: Domain-first deployment — no server IP hardcoded in runtime
All runtime configuration (nginx `server_name`, CORS origins, `NEXT_PUBLIC_SITE_URL`, bot webhook URLs) uses domain names (`ab-event.pro`, `www.ab-event.pro`, `test.ab-event.pro`), never a server IP. The server IP is referenced only in deployment documentation and DNS setup instructions, always as the placeholder `<NEW_TIMEWEB_SERVER_IP>`. This allows server migration (including Timeweb Cloud VPS replacement) without any code or config change — only DNS A-records need to be updated. The old server IP `77.232.136.248` is preserved in historical TZ documents for reference but must not be reused. When a new server IP is assigned, update only: (1) DNS A-records in the DNS provider panel, (2) deployment documentation placeholders.
