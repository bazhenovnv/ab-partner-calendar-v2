...
## [Unreleased] — 2026-08-03 — Multiple reminder dates in bots

### Изменено

- В Telegram и MAX ручной ввод даты и времени заменён выбором одной или нескольких дат кнопками.
- После выбора даты подтверждаются одной кнопкой «Применить»; напоминания создаются на 09:00 по московскому времени.
- MAX-интеракции перенесены в действующий webhook backend, чтобы выбор дат работал при активной подписке на события MAX.
- Добавлены общие расчёты доступных дат и автоматические тесты сценариев Telegram, MAX и нормализации webhook-событий.

---

## [Unreleased] — 2026-07-31 — Production visual baseline

### Зафиксировано

- `main` приведён к визуальному варианту, подтверждённому на production 31 июля: frontend-геометрия и стили от последней версии 29 июля.
- Верхняя панель использует отслеживаемый `hero-composition.png`; удалены ссылки на отсутствующие `hero-*-approved.png`.
- Иконки Telegram, MAX и «Стать партнёром» закреплены как встроенные SVG и больше не зависят от отсутствующих `icon-*-approved.png`.
- Полные тексты пяти юридических документов и канонический frontend fallback сохранены.
- Добавлены smoke-проверки, запрещающие возврат отсутствующих assets и неверного визуального слоя.

---

## [Unreleased] — 2026-07-30 — Legal documents and desktop scale

### Добавлено

- Опубликованы полные тексты пяти юридических документов из пакета `AB_Afisha_legal_and_TZ_v11_package.zip`.
- Добавлена идемпотентная миграция БД, заменяющая ранее опубликованные заглушки, создающая документы при их отсутствии и записывающая опубликованные версии в историю.
- Канонические тексты подключены как frontend fallback и как источник для seed.

### Изменено

- Desktop presentation scale увеличен ровно на 10% относительно текущего значения: `0.80 → 0.88`.
- Компенсационная ширина desktop-wrapper пересчитана: `125% → 113.6364%`.
- Добавлены адаптивные стили таблиц юридических документов.
