...
| Figma editor workspace vs frame canvas | The medium-gray (~`#BBBBBB`) visible around the design in Figma editor screenshots was confused with the page canvas in Stage 58.3 investigation | The page canvas `#F1F1F1` is the fill INSIDE the selected 1920×3565 frame (`5913:4745`); the darker gray is Figma editor UI outside the frame | Direct color verification inside the selected frame (Stage 58.4) | RESOLVED. Rule added to `09_PAGE_CANVAS_AND_SCALE.md`: sample only inside the blue frame boundary. Never implement `#BBBBBB` or any darker workspace gray. |
