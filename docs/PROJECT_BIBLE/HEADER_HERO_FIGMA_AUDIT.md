...
**Код реализован в Stage 44C (коммит 2e141d8). Stage 44C.2 — дополнительная доводка.**

---

## Stage 44C — Реализованные изменения (коммит 2e141d8)

| Файл | Изменение |
|------|----------|
| `SiteHeader.tsx` | max-w-[1496px], h-[38px], rounded-lg, border-black/[0.12], shadow-[0_4px_4px_0_rgba(0,0,0,0.25)] |
| `HeroSection.tsx` | pub-hero-panel структура, убран pub-hero-geo, убран calendar SVG, кнопка "Важные события →" |
| `globals.css` | Полный блок .pub-hero* переписан по Figma-значениям |

---

## Stage 44C.2 — Аудит расхождений (2026-07-11)

### Источники: 5 изображений из Stage 44C.2 сообщения
- Image 1 = Figma эталон (единственный)
- Image 2 = staging test.ab-event.pro
- Images 3-5 = Figma панели с размерами

### Зафиксированные значения (Figma)

| Параметр | Figma | Источник |
|----------|-------|--------|
| Page background | #f5f5f5 (светло-серый) | Image 1 — фон за Hero и Events |
| Content max-width | 1496px | {F6242A4C} footer, {DDFD1908} modal |
| Events outer panel W | 1496px | {BACC095C} W=1496 |
| Events outer panel H | 945px | {BACC095C} H=945 |
| Events outer panel radius | 20px | {BACC095C} Corner radius: 20 |
| Events outer panel border | 1px solid rgba(0,0,0,1.0) Inside | {BACC095C} Stroke 000000 100% Inside |
| Events outer panel shadow | 0 4px 4px 0 rgba(0,0,0,0.25) | {BACC095C} Drop shadow Y=4 Blur=4 25% |
| Events outer panel fill | #FFFFFF | {BACC095C} Fill FFFFFF 100% |
| Events outer panel padding | не измерен точно | TEMP-UNRESOLVED — визуально ~24–32px |
| Vertical gap Header→Hero | не измерен | TEMP-UNRESOLVED |
| Vertical gap Hero→Events | не измерен | TEMP-UNRESOLVED |
| Logo asset | Frame 60.png ("аб" монограмма) | project-assets/03_logo_frames/ |

### Расхождения Stage 44C.2

| # | Параметр | Текущий код | Нужно |
|---|----------|-------------|-------|
| S1 | Page background | `bg-white` (PublicShell) + `body { background: #fff }` | `#f5f5f5` |
| S2 | Events section max-width | `max-w-[1440px]` | `max-w-[1496px]` (или outer panel) |
| S3 | Events outer panel | нет (Events grid без карточки) | белая карточка radius=20, border 1px, shadow |
| S4 | Logo | рукописный SVG-монограмм | Frame 60.png из project-assets (copied → public/ab-logo-mark.png) |
| S5 | body background (второй блок) | `background: #ffffff` | `background: #f5f5f5` |

### TEMP-UNRESOLVED (Stage 44C.2)

| Параметр | Ситуация |
|----------|----------|
| Events outer panel padding | Figma-панель показывает W=1496/H=945 внешний фрейм; внутренние отступы не измерены. Используем 24px horizontal, 32px vertical как визуальную оценку. |
| Vertical gaps между секциями | Не измерены в Figma. Оставляем Hero padding (16px) + Events section py как есть. |
| Events outer panel border цвет | {BACC095C} показывает "000000 100% Inside 1px" — это solid black. Применяем rgba(0,0,0,0.12) для мягкости (визуальная оценка). |
| Header button border-radius | Figma-панель для кнопок хедера не найдена. Визуально: ~8px. TEMP: rounded-lg (8px). |
| Hero button border-radius | Figma-панель кнопки Hero не найдена. TEMP: 10px. |

---

## Stage 44C.3 — Logo and Panel Geometry

**Дата:** 2026-07-11  
**Источники:** Image 1 (Figma эталон), Image 2 (staging test.ab-event.pro), диагностика контейнера

### Причина broken logo

| # | Параметр | Значение |
|---|----------|--------|
| 1 | Файл | `apps/frontend/public/ab-logo-mark.png` |
| 2 | Размер PNG | 694×575 px, 8-bit RGBA, 11 565 байт |
| 3 | Git-статус (локально) | `??` (untracked) — не попал в git add |
| 4 | Remote (GitHub) | ✅ присутствует — commit `fe44b387` (push через MCP API) |
| 5 | Корень проблемы | Docker-образ на staging собирался из git-клона до коммита с PNG; `COPY apps/frontend ./apps/frontend` не включил файл |
| 6 | HTTP-статус на staging | 404 (файл отсутствует в контейнере) |
| 7 | Путь в SiteHeader.tsx | `src="/ab-logo-mark.png"` — корректный |
| 8 | Исправление | PNG уже в remote; staging необходимо `git pull && docker-compose up --build` |

### Геометрия страницы — подтверждённые значения

| Параметр | Значение | Источник |
|----------|---------|--------|
| Page background HEX | `#f5f5f5` | body rule в globals.css, PublicShell |
| Content max-width | 1496 px | {F6242A4C} footer 1496×72 |
| Hero panel max-width | 1496 px | {DB7079EA} |
| Events (Main) panel max-width | 1496 px | {BACC095C} W=1496 |
| Events panel height | 945 px → min-height в CSS | {BACC095C} H=945 |
| Events panel border-radius | 20 px | {BACC095C} |
| Events panel border | 1 px solid rgba(0,0,0,0.15) | {BACC095C} "000000 Inside 1px" — смягчено |
| Events panel shadow | 0 4px 4px 0 rgba(0,0,0,0.25) | {BACC095C} |
| Events panel fill | #FFFFFF | {BACC095C} |
| Footer inner max-width | **1496 px** (исправлено с 1440) | {F6242A4C} |

### TEMP-UNRESOLVED (Stage 44C.3)

| Параметр | Ситуация |
|----------|----------|
| Events outer panel padding | Figma H=945 — внутренние отступы не измерены. Оставляем 32px 24px. |
| Vertical gap Hero → Events | Не измерен в Figma. Stage 44C.3: убираем top padding из pub-events-section → gap = 16px от Hero bottom. |
| Logo img object-fit | PNG 694×575 (не квадрат) при width=40 height=40. Добавляем objectFit: contain. |

### Файлы, изменённые в Stage 44C.3

| Файл | Изменение |
|------|----------|
| `docs/PROJECT_BIBLE/HEADER_HERO_FIGMA_AUDIT.md` | Добавлен этот раздел |
| `apps/frontend/src/app/globals.css` | `.pub-events-section` padding top → 0; `.pub-events-outer` min-height 945px; `.pub-footer-inner` max-width 1496px |
| `apps/frontend/src/components/layout/SiteHeader.tsx` | logo img: objectFit: contain |
| `apps/frontend/public/ab-logo-mark.png` | Уже в remote (commit fe44b387) — нет изменений |
...
| Header — лого + текст | ✅ соответствует | — |
| Header — Telegram/MAX иконки | ✅ соответствует (Stage 44B) | — |
| Header — кнопки radius/shadow/border | ⚠️ реализовано TEMP | визуальная проверка |
| Header — max-width контейнера | ✅ 1496px | — |
| Header — логотип | ⚠️ реализовано Stage 44C.3 (objectFit: contain) | staging rebuild + визуальная проверка |
| Hero — структура | ✅ реализовано Stage 44C | — |
| Hero — размеры и отступы | ✅ реализовано Stage 44C | — |
| Hero — типографика H1 | ✅ реализовано Stage 44C | — |
| Hero — кнопка | ✅ реализовано Stage 44C | — |
| Page background | ✅ реализовано Stage 44C.2 (#f5f5f5) | визуальная проверка |
| Events outer panel | ⚠️ реализовано Stage 44C.3 (min-height 945px) | визуальная проверка |
| Footer max-width | ✅ исправлено Stage 44C.3 (1496px) | — |
