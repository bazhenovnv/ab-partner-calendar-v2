...
## ⚠️ Актуальный сервер (обновлено 2026-07-02)

> **Старый сервер удалён.** IP `77.232.136.248` (host `kvnvm-277`) больше не существует.
> Все действия выполняются только на новом сервере.
...
| WWW | `www.ab-event.pro` |
| Staging | `test.ab-event.pro` (HTTP-only до выпуска SSL) |

### DNS A-записи для нового сервера

После получения IP от Timeweb Cloud установить следующие A-записи у DNS-провайдера:
...
После создания нового VPS убедиться, что базовое ПО установлено:
...
Если Docker не установлен — выполнить раздел **2.3** ниже.
Если certbot не установлен — выполнить раздел **2.4** ниже.
