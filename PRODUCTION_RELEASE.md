...
- если полное имя city уже присутствует в address, город повторно не добавляется; иначе строка формируется как `Город, адрес`;
...
Application PR #170 / CI #965 устраняет edge case проверки города перед production promotion:

- raw substring matching больше не используется для определения присутствия city в address/venue;
- punctuation и дефисы нормализуются до пробелов, после чего проверяется целая нормализованная фраза с границами;
- `Москва` корректно распознаётся внутри `г. Москва, ...`;
- `Санкт-Петербург` и `Санкт Петербург` сопоставляются после нормализации;
- `Омск` **не** считается присутствующим внутри `Омская улица`, поэтому корректный результат будет `Омск, Омская улица, 10`;
...
- если address уже содержит полное имя city, город не дублируется;
- проверить edge case: city `Омск`, address `Омская улица, 10` должен выводиться как `Омск, Омская улица, 10`;
