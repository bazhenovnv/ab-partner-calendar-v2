...
Машиночитаемая фиксация находится в `infra/deploy/production-frontend.env`. Production-компоненты закрепляются независимо: release anchor остаётся на backend `213e507`, а frontend продвигается отдельно до `460b50c`.

## Текущая promotion — полный адрес и спикер в модальном окне

Текущая promotion меняет **только frontend** до application merge commit `460b50cc7fed664ccf150c3bb35a3ae2bcce4750`.
...
- frontend меняется только на `ab-afisha/frontend:frontend-release-460b50c`;
...
2. собрать frontend из commit `460b50cc7fed664ccf150c3bb35a3ae2bcce4750` в detached worktree;
...
- frontend image = `ab-afisha/frontend:frontend-release-460b50c`;
- frontend revision = `460b50cc7fed664ccf150c3bb35a3ae2bcce4750`;
...
- frontend release кроме `frontend-release-460b50c`;
