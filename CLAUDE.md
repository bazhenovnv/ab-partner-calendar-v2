...
Текущая promotion — **frontend-only**. Application commit `9e37708` включает PR #144, финально проверенный CI #891. На мобильном viewport серый canvas восстановлен под изображениями ног в блоке цитат, белая полоса непосредственно под цитатой сохранена и получила равномерную верхнюю/нижнюю тень; `Контакты` оптически выровнены с `Наши проекты`; название `Афиша бухгалтера` немного опущено относительно логотипа; блокнот/растение слегка увеличены без выхода за правую границу и без появления фрагмента отдельной чашки. Desktop не меняется.

Текущий контракт deployment:

- backend `213e507` сохраняется и не пересоздаётся;
- frontend меняется только на `frontend-release-9e37708`;
...
Сохраняется PR #142 / CI #887 по compact-карусели «Главные события»: боковые карточки выровнены без `rotateY` и `rotateZ`, при этом существующие translate/scale/opacity/brightness/blur/z-index и эффект глубины сохраняются.
