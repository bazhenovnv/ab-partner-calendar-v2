...
const COUNTRY_VALUES = new Set(['россия', 'рф', 'russia']);
...
function findCatalogueCity(values: string[], catalogueCityNames: string[]): string | null {
...
function extractCityFromValue(value: string): string | null {
  ...
  const explicitCity = trimmed.match(/(?:^|[,;])\s*(?:г\.|город)\s*([^,;]+)/i);
...
export function extractRegionFromEventLocation(source: EventLocationSource): string | null {
  const parts = [source.address, source.cityName]
    .filter((value): value is string => Boolean(value?.trim()))
    .flatMap((value) => splitLocationParts(value));

  for (let index = parts.length - 1; index >= 0; index -= 1) {
    const candidate = parts[index].trim();
    if (looksLikeRegion(candidate)) return candidate;
  }
